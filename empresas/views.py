...
def ClientView(request):
	...
	recommended_clients = company.get_recommended_clients_v2() #company.recommended.all()
		...
		if region=="true" or sector!="": 
			if region=="true":
				recommended_clients = recommended_clients.exclude(
					territorial=company.territorial)
			else:
			...
			recommended_clients = recommended_clients.exclude(name=company.name)
			context = {
				"company": company,
				"recommended_clients": recommended_clients[:50],
				"loading_times": request.session['recommended_clients_page'],
